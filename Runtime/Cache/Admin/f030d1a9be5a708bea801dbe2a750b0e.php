...
<div class="page-container">
	<div class="text-c">
		<form class="Huiform" method="get" action="/admin.php/admin/powers.html" target="_self">
	...
	<table class="table table-border table-bordered table-bg">
		<thead>
			<tr>
				<th scope="col" colspan="7"><?php echo ($bread['curName']); ?></th>
			</tr>
			<tr class="text-c">
				<th width="40">ID</th>
				<th width="200">模块</th>
		...
		<tbody>
			<?php if(is_array($powers['list'])): $i = 0; $__LIST__ = $powers['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
				...
				<?php if(!empty($vo['sub'])): if(is_array($vo['sub'])): $i = 0; $__LIST__ = $vo['sub'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sub): $mod = ($i % 2 );++$i;?><tr class="text-c">
						...
						<td>
                            <a title="编辑" href="javascript:;" onclick="admin_permission_edit('权限编辑','<?php echo U('admin/powers_edit?pid='.$sub['id']);?>','','','400')" class="ml-5" style="text-decoration:none">
                                <i class="Hui-iconfont">&#xe6df;</i>
                            </a>
                        </td>
