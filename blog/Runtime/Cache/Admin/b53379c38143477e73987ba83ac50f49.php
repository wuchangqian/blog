...
<div class="page-container">
	<div class="text-c">
		<form class="Huiform" method="get" action="/Admin/admin/users.html" target="_self">
			<input type="text" class="input-text" style="width:250px" placeholder="用户名|登录IP" value="<?php echo ($keyword); ?>" name="keyword">
			<button type="submit" class="btn btn-success" id="" name=""><i class="Hui-iconfont">&#xe665;</i> 搜权用户</button>
		</form>
	</div>
	...
	<table class="table table-border table-bordered table-bg">
		<thead>
			<tr>
				<th scope="col" colspan="8"><?php echo ($bread['curName']); ?></th>
			</tr>
			<tr class="text-c">
				<th width="40">ID</th>
				<th width="200">用户名</th>
		...
		<tbody>
			<?php if(is_array($users['list'])): $i = 0; $__LIST__ = $users['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
					...
					<td>
                        <a title="编辑" href="javascript:;" onclick="admin_permission_edit('权限编辑','<?php echo U('admin/powers_edit?pid='.$vo['id']);?>','','','400')" class="ml-5" style="text-decoration:none">
